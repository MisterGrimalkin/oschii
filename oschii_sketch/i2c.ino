...
void startI2CGpio() {
  i2c.start_gpio(SDA_PIN, SCL_PIN);
}
...
void flushI2CGpioCache() {
  i2c.write_gpio(I2C_ADDR, I2C_DATA_ADDR[0], i2COut[0]);
  i2c.write_gpio(I2C_ADDR, I2C_DATA_ADDR[1], i2COut[1]);
}
...
void setI2CGpioDirection(int port, int pin, bool output) {
  ...
  i2c.write_gpio(I2C_ADDR, I2C_IODIR_ADDR[port], i2CDir[port]);
}
...
void setI2CGpioResistance(int port, int pin, bool up) {
  ...
  i2c.write_gpio(I2C_ADDR, I2C_RESISTANCE_ADDR[port], i2CRes[port]);
}
...
void pullI2CGpio() {
  i2CIn[0] = i2c.read_gpio(I2C_ADDR, I2C_DATA_ADDR[0]);
  i2CIn[1] = i2c.read_gpio(I2C_ADDR, I2C_DATA_ADDR[1]);
}
